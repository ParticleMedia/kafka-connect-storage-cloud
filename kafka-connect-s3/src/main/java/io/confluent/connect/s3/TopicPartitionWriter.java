...
public class TopicPartitionWriter {
  ...
  private final HashSet<String> commitFilesName = new HashSet<String>();
  ...
  TopicPartitionWriter(TopicPartition tp,
    ...
    pathFormat = connectorConfig.getString(PartitionerConfig.PATH_FORMAT_CONFIG);
    partitionDurationMs = connectorConfig.getLong(PartitionerConfig.PARTITION_DURATION_MS_CONFIG);
    timeZoneString = connectorConfig.getString(PartitionerConfig.TIMEZONE_CONFIG);
...
//      return true;
  ...
  private void commitFiles() {
    ...
    for (Map.Entry<String, String> entry : commitFiles.entrySet()) {
      ...
      String pathPrefix = getDirectoryPrefix(encodedPartition) + dirDelim;
      commitFilesName.add(pathPrefix);
    ...
    String nowPrefix = tp.topic() + dirDelim + encodeTimestamp(
            partitionDurationMs,
            pathFormat,
            timeZoneString,
            time.milliseconds());
    for (String commitFileName : commitFilesName) {
      ...
      if (!commitFileName.equals(nowPrefix)) {
        ...
        try {
          ...
          log.error("error! commitFileName {}! nowPrefix {}!", commitFileName, nowPrefix);
          throw new RetriableException(e);
        }

      }
    }
    commitFilesName.clear();
